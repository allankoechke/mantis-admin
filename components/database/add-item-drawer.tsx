...
export function AddItemDrawer({ table, apiClient, open, onClose, onItemAdded }: EditItemDrawerProps) {
  ...
  React.useEffect(() => {
    if (open) {
      ...
      setTableFields(table.schema?.fields.filter( f => !(f.system && ["id", "created", "updated"].includes(f.name))))
    }
  }, [open])
              ...
              {tableFields?.map((field : any) => (
                <div key={field.name} className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled={isSystemField(field) && ["id", "created", "updated"].includes(field)}
                    className={`w-full ${(isSystemField(field) && ["id", "created", "updated"].includes(field)) ? "bg-muted" : ""}`}
